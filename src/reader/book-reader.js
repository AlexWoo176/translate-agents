document.addEventListener('DOMContentLoaded', () => {
  ...
  function parseCSV(text) {
    ...
    // Normalize headers for term -> key mapping
    const normalizedHeaders = headers.map(h => h === 'term' ? 'key' : h);
          ...
          if (matchedTerm) {
            termDetailsContainer.innerHTML = `
              <div class="br-term-card">
                ...
                <div class="br-desc-en"><strong>EN Desc:</strong> ${matchedTerm.desc_en || matchedTerm.context || 'N/A'}</div>
                <div class="br-desc-vi"><strong>VN Desc:</strong> ${matchedTerm.desc_vi || matchedTerm.notes || 'N/A'}</div>
              </div>
            `;
          } else {
            termDetailsContainer.innerHTML = `
